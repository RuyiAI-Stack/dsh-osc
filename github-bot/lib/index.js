...
//#endregion
//#region src/tools/api/create-comment.ts
async function createComment(config, input) {
	const orgConfig = assertOrgRepo(input, config.orgs);
	const [owner, repo] = input.repo.split("/");
	if (!Number.isSafeInteger(input.number) || input.number <= 0) throw new Error("github-bot: issue or pull request number must be positive");
	if (typeof input.body !== "string" || input.body.length === 0) throw new Error("github-bot: body is required");
	const { token } = await createInstallationToken(input.org, orgConfig);
	const comment = await githubRequest(orgConfig, token, `/repos/${owner}/${repo}/issues/${input.number}/comments`, {
		method: "POST",
		body: JSON.stringify({ body: input.body })
	});
	if (!Number.isSafeInteger(comment.id) || typeof comment.html_url !== "string" || comment.html_url.length === 0) throw new Error("github-bot: GitHub comment response is missing id or html_url");
	return {
...
function defineCreateCommentTool(bot) {
	return defineTool({
		...
		execute: async (args) => createComment(bot.config, args)
	});
}
...
var GitHubBot = class extends Service {
	...
	createComment(input) {
		return createComment(this.config, input);
	}
};

//#endregion
export { assertCommitInput, assertOrgRepo, commitBranch, createComment, createIssue, GitHubBot as default, openPullRequest };